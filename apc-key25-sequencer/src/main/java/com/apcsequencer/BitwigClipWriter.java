...
public final class BitwigClipWriter implements ClipWriter {
     ...
     * @param host  used for diagnostic logging only
     */
    public BitwigClipWriter(PinnableCursorClip[] clips, SequencerState state, ControllerHost host) {
        ...
        for (int t = 0; t < SequencerState.TRACK_COUNT; t++) {
            ...
            // Mark interest so the exists() callback fires reliably.
            clip.exists().addValueObserver(exists -> {
                if (exists) {
                    // Drain any writes that arrived before the clip was ready.
